...
public final class Debugger {
        ...
        if (element instanceof Mesh && element.getCullHint() != Spatial.CullHint.Always) {
            ...
            if (norms != null && verts != null && norms.limit() == verts.limit()) {
                ...
                for (int x = 0; x < geom.getMeshData().getVertexCount(); x++) {
                    ...
                    lineVerts.put((float) _normalVect.getX());
                    lineVerts.put((float) _normalVect.getY());
                    lineVerts.put((float) _normalVect.getZ());
    ...
    public static void drawBuffer(final Texture.RenderToTextureType rttSource, final int location, final Renderer r,
            final float size) {
        r.flush();
        double locationX = r.getWidth(), locationY = r.getHeight();
        ...
        if (bufTexRend == null) {
            final DisplaySettings settings = new DisplaySettings(256, 256, 0, 0, 0, 8, 0, 0, false);
            bufTexRend = TextureRendererFactory.INSTANCE.createTextureRenderer(settings, r,
