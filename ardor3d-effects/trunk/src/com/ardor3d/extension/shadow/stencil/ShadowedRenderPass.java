...
public class ShadowedRenderPass extends Pass {
    ...
    public void doRender(final Renderer r) {
        ...
        for (int l = shadowLights.size(); --l >= 0;) {
            ...
            if (context.getCapabilities().isTwoSidedStencilSupported()) {
    ...
    protected void renderScene(final Renderer r) {
        for (int i = 0, sSize = spatials.size(); i < sSize; i++) {
            final Spatial s = spatials.get(i);
            s.onDraw(r);
        }
        r.renderQueue();
    }
    ...
    protected void init(final Renderer r) {
        if (initialised) {
            return;
        }
    ...
    public void resetShadowQuad(final Renderer r) {
        if (r.getWidth() == quadWidth && r.getHeight() == quadHeight) {
            return;
        }
